...
class ChatRecoveryTests(unittest.TestCase):
    ...
    def test_deadline_does_not_prevent_verifying_an_already_completed_write(self):
        ...
        with patch("chat_ops.engine.time.time", return_value=workflow["deadline"] + 1):
            # Main deliberately requires explicit read-only reconciliation after expiry.
            self.assertEqual(self.e.run("alpha", run, tools)["state"], "UNCERTAIN")
            readback = self.e.reconcile("alpha", run, "issue")
            self.assertTrue(readback["invocation"]["read_only"])
            self.e.record("alpha", run, "issue", readback["token"], tools.invoke(readback["invocation"]))
    ...
    def test_same_connector_supports_multiple_explicit_accounts(self):
        ...
        # Preserve main's named endpoint/resource configuration, not obsolete internals.
        transport = MCPTransport({"first": {**make("a"), "connector": "github"},
                                  "second": {**make("b"), "connector": "github"}})
        self.assertEqual({client.config["account_ref"] for client in transport.clients.values()}, {"a", "b"})
        with self.assertRaises(ContractError):
            MCPTransport({"first": {**make("a"), "connector": "github"},
                          "duplicate": {**make("a"), "connector": "github"}})
        with self.assertRaises(ContractError):
            transport.invoke({"connector": "github", "account_ref": "unknown", "resource": "repo"})
