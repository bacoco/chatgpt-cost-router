...
def build_server(config_path: str | None = None):
    ...
    @mcp.tool(title="Run remote command", description="Execute allowlisted argv on one host over SSH. This can modify the host. Only bounded inspection is accepted; use a named project-bound process profile for changes.", annotations=annotations(False))
    ...
    @mcp.tool(title="Fast-forward remote repository", description="Legacy endpoint: raw Git pulls are blocked; use an operator-enrolled process profile.", annotations=annotations(False))
