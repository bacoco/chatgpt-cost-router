...
def build_server(config_path: str | None = None):
    ...
    @mcp.tool(title="Run remote command", description="Execute allowlisted argv on one host over SSH. This can modify the host. Only bounded read commands or fast-forward Git pull are accepted; use a named process profile for general computation.", annotations=annotations(False))
    ...
    @mcp.tool(title="Fast-forward remote repository", description="Run git pull --ff-only in an allowed repository path on one host.", annotations=annotations(False))
